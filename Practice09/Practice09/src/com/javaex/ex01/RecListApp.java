...
public class RecListApp {
	...
	public static void main(String[] args) {
		...
		//사각형(4,5) 수정 코드 작성	
		rList.getLast().setHeight(9);
		rList.getLast().setWidth(5);
		...
		Rectangle rectangle;
		for (Iterator<Rectangle> iterator = rList.iterator(); iterator.hasNext();) {
	...
	public static void listAllDraw(List<Rectangle> list) {
		...
		//내용작성 
		for (Iterator<Rectangle> iterator = list.iterator(); iterator.hasNext();) {
			rectangle = (Rectangle) iterator.next();
			rectangle.draw();
		}
	}
